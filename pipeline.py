...
class Tracker():
    ...
    def get_boxes(self, img):
        ...
        for y_range, win_size in zip(self.y_ranges, self.win_sizes):
            ...
            hog1 = pf.get_hog_features(ch1_ycc, self.hog_params.orient, self.hog_params.pix_per_cell, self.hog_params.cells_per_block, feature_vec=False)
            hog2 = pf.get_hog_features(ch2_ycc, self.hog_params.orient, self.hog_params.pix_per_cell, self.hog_params.cells_per_block, feature_vec=False)
            hog3 = pf.get_hog_features(ch3_ycc, self.hog_params.orient, self.hog_params.pix_per_cell, self.hog_params.cells_per_block, feature_vec=False)
            ...
            if use_two_colors:
                ...
                hog4 = pf.get_hog_features(ch1_luv, self.hog_params.orient, self.hog_params.pix_per_cell, self.hog_params.cells_per_block, feature_vec=False)
                hog5 = pf.get_hog_features(ch2_luv, self.hog_params.orient, self.hog_params.pix_per_cell, self.hog_params.cells_per_block, feature_vec=False)
                hog6 = pf.get_hog_features(ch3_luv, self.hog_params.orient, self.hog_params.pix_per_cell, self.hog_params.cells_per_block, feature_vec=False)
    ...
    def process_frame(self, img):
        ...
        if self.debug == False:
            ...
            pf.apply_threshold(heatmap_strong, 5)
            heatmap = heatmap_strong+heatmap_med15
            labels = label(heatmap)
            car_boxes_labeled = pf.get_labeled_bboxes(labels)
...
def get_data(orientations, pixels_per_cell, cells_per_block):
    ...
    v_train_names, v_test_names, nv_train_names, nv_test_names = pf.read_images(train_split = 0.8)
    ...
    for img_name in img_train_names:
        ...
        img_features = pf.get_features(img, orientations, pixels_per_cell, cells_per_block)
