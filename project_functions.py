...
def read_images(train_split = 0.7):
    ...
    vehicles_train = []
    vehicles_test = []
    for path in os.listdir('train_images/vehicles/vehicles/'):
        path = 'train_images/vehicles/vehicles/' + path
    ...
    non_vehicles_train = []
    non_vehicles_test = []
    for path in os.listdir('train_images/non-vehicles/non-vehicles/'):
        path = 'train_images/non-vehicles/non-vehicles/' + path
        if os.path.isdir(path):
            all_files = glob(path+'/*.png')
            num_train = int(len(all_files)*train_split)
